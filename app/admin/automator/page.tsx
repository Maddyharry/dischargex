...
import { incidentSchema } from "@/lib/automator-contract";
function displayReport(payload: string | null) {
  try {
    const parsed = incidentSchema.safeParse(JSON.parse(payload || "null"));
    return parsed.success ? JSON.stringify(parsed.data, null, 2) : "รายงานรูปแบบเก่า หรือข้อมูลไม่สมบูรณ์";
  } catch { return "อ่านรายงานนี้ไม่ได้ รายงานอื่นยังเปิดดูได้ตามปกติ"; }
}
...
export default async function AutomatorSupport() {
  ...
  if(user?.role!=="admin")return <main className="p-8">กรุณาเข้าสู่ระบบผู้ดูแล</main>;
